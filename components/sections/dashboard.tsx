...
import { TrendingUp, TrendingDown, Target, Flame, CalendarDays, SkipForward, DollarSign, Coins, ArrowUpRight, ArrowDownRight } from "lucide-react"
...
export function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const lastTrade = trades.length > 0 ? [...trades].sort((a, b) => b.date.localeCompare(a.date))[0] : null
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {/* Last Trade Card */}
      <GlassCard className="p-4 overflow-hidden relative group" hover3d>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className={cn(
              "w-10 h-10 rounded-xl flex items-center justify-center",
              stats.lastTrade ? (stats.lastTrade.result > 0 ? "bg-success/15" : "bg-destructive/15") : "bg-muted/50"
            )}>
              {stats.lastTrade ? (
                stats.lastTrade.result > 0 ? <TrendingUp className="w-5 h-5 text-success" /> : <TrendingDown className="w-5 h-5 text-destructive" />
              ) : <Target className="w-5 h-5 text-muted-foreground" />}
            </div>
            <div>
              <div className="text-[10px] text-muted-foreground font-medium uppercase">Last Trade</div>
              <div className="text-sm font-bold text-foreground">
                {stats.lastTrade ? stats.lastTrade.asset : "No trades yet"}
              </div>
            </div>
          </div>
          {stats.lastTrade && (
            <div className="text-right">
              <div className="text-[10px] text-muted-foreground">{new Date(stats.lastTrade.date).toLocaleDateString()}</div>
              <div className={cn("text-sm font-black", stats.lastTrade.result > 0 ? "text-success" : "text-destructive")}>
                {formatPercent(stats.lastTrade.result)}
              </div>
            </div>
          )}
        </div>
